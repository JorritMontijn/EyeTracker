...
for intRunPrePro=1:size(matRunPre,1)
	...
	%else
		strThisRecIdx = getFlankedBy(cellSourcePath{end},'R','_','last');
		strRec = ['*R' strThisRecIdx '*'];
	%end
	strThisMouseIdx = getFlankedBy(cellSourcePath{end},'_M','_','last');
	if ~isempty(strThisMouseIdx)
		strRec = [strRec strThisMouseIdx '*'];
	else
		strRec = '*';
	end
	...
	while hasFrame(objVid)
		...
		%find pupil
		[sPupil,imPupil,imReflection,imBW,imGrey] = getPupil(gMatVid,gMatFilt,sglReflT,sglPupilT,objSE,vecPrevLoc,vecPupilT,sET);
